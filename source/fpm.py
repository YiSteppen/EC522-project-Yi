...
class fpm(nn.Module):
    def __init__(self, Np, na, na_illum, na_list, wl, ps, mag, alpha, maxIter, measurements, C_init = None, testFlag = False, device='cpu'):
        ...
        self.P = torch.stack((torch.from_numpy(P_tmp.real.copy()), torch.from_numpy(P_tmp.imag.copy())), dim=2)
    ...
    def grad(self, field_est, device='cpu'):
        ...
        output = torch.fft.fft2(output,dim = (1,2))
        output = mul_c(self.P,output)
        output = torch.fft.ifft2(output,dim = (1,2))
        plt.figure()
        plt.imshow(np.abs(output[0,...,0].cpu().numpy()))
        g = field_est*0.
        for meas_index in range(self.Nmeas):
            output2 = mul_c(Ajx_c[meas_index,...],output)
            output2 = mul_c(conj(self.planewaves),output2)
            output2 = torch.fft.fft2(output2,dim = (1,2))
            output2 = mul_c(self.pupils,output2)
            output2 = torch.fft.ifft2(output2,dim = (1,2))
            g_tmp = torch.matmul(torch.real(output2).permute(1,2,3,0),self.C[meas_index,:])
            g = g + g_tmp
#         return -1 * self.alpha * g, cost
    ...
    def generateSingleMeas(self,field,device="cpu"):
        ...
        print('output',output.shape)
        output = torch.fft.fft2(output,dim = (1,2))
        output1 = torch.fft.fft2(output,dim = (1,2))
        output2 = torch.fft.fft(torch.fft.fft(output,dim = 1),dim = 2)
        plt.figure()
        plt.imshow(np.abs(output1[0,...,0].cpu().numpy()))
        plt.figure()
        plt.imshow(np.abs(output2[0,...,0].cpu().numpy()))
        output = mul_c(self.P, output)
        output = torch.fft.ifft2(output,dim = (1,2))
        output1 = torch.fft.ifft2(output,dim = (1,2))
        output2 = torch.fft.ifft(torch.fft.ifft(output,dim = 1),dim = 2)
        plt.figure()
        plt.imshow(np.abs(output1[0,...,0].cpu().numpy()))
        plt.figure()
        plt.imshow(np.abs(output2[0,...,0].cpu().numpy()))
        ...
        print('output',output.shape)
        plt.figure()
        plt.imshow(np.abs(output[0,...].cpu().numpy()))
        return output
    ...
    def generateMultiMeas(self,field,device="cpu"):
        ...
        output = mul_c(self.planewaves,field)
        output = torch.fft.fft(output,2)
        output = mul_c(self.P, output)
        output = torch.fft.ifft(output,2)
        output = abs2_c(output)
        output_real = torch.real(output)
        C_real = torch.real(self.C)

        # Convert tensors to complex
        output_complex = torch.complex(output_real, torch.zeros_like(output_real))
        C_complex = torch.complex(C_real, torch.zeros_like(C_real))
        multiMeas = torch.matmul(output_complex.permute(1,2,0), C_complex.permute(1,0)).permute(2,0,1)
        return multiMeas
    ...
    def cropMeasurements(self, measurements):
        ...
        for img_idx in range(measurements.shape[0]):
            print('cropMeasurements')
            fmeas = utility.fftshift2(torch.fft.fft2(measurements[img_idx,...],dim = (1,2)))
    ...
    def padMeasurements(self, measurements):
        ...
        for ii in range(measurements.shape[0]):
            fmeas = utility.fftshift2(torch.fft.fft2(measurements[ii,...],dim = (1,2)))
    ...
    def setup_illumination(self,):
        ...
        for ii in range(self.Nleds):
            print('planewaves',planewaves.shape)
